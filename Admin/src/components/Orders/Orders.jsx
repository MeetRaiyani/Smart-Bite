...
const Order = () => {
  ...
  const fetchData = async () => {
    let response;
    await fetch("http://localhost:8000/api/order/list")
      .then((res) => res.json())
      .then((data) => {
        response = data;
      })
  ...
  return (
    <div className="w-full h-[100vh] overflow-y-auto  p-4 rounded-lg">
      <div className="flex items-center justify-center w-full text-[20px] mb-4 font-poppins font-semibold">
        <h1 className=" font-poppins font-semibold text-3xl">Orders List</h1>
      </div>
